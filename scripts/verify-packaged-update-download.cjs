...
const oldInstallerPath =
  process.env.HBCLIENT_CURRENT_INSTALLER || path.resolve("apps", "desktop", "release", "HBClient-0.1.2-win-x64.exe");
...
const oldBlockmapPath = fs.existsSync(configuredOldBlockmapPath)
  ? configuredOldBlockmapPath
  : path.join(fixtureDir, oldBlockmapName);
const newBlockmapPath = fs.existsSync(configuredNewBlockmapPath)
  ? configuredNewBlockmapPath
  : path.join(fixtureDir, newBlockmapName);
...
async function main() {
  ...
  try {
    ...
    if (
      !stats.blockmapRequests.includes(`/${oldBlockmapName}`) ||
      !stats.blockmapRequests.includes(`/${newBlockmapName}`)
    ) {
