...
const appPath = path.resolve("apps", "desktop", "release", "win-unpacked", "HyWork.exe");
const port = 9333;
...
async function main() {
  ...
  const railToggleCount = Number(
    await evaluate(page.webSocketDebuggerUrl, "document.querySelectorAll('.rail-toggle').length"),
  );
  const topbarToggleCount = Number(
    await evaluate(
      page.webSocketDebuggerUrl,
      "document.querySelectorAll('.topbar-actions button .anticon-menu-fold, .topbar-actions button .anticon-menu-unfold').length",
    ),
  );
  ...
  const composerFixed =
    composerBox && composerBox.position === "fixed" && Math.abs(composerBox.bottom - composerBox.viewport) <= 2;
  ...
  if (
    ...
    throw new Error("Packaged HyWork window did not load the expected updated UI.");
  }
}
