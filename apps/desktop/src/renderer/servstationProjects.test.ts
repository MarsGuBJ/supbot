...
describe("Server Agent projects", () => {
  ...
  it("returns only the selected conversation transcript and keeps drafts empty", () => {
    ...
    expect(servstationMessagesForConversation([historyA, historyB], historyA.id).map((message) => message.text)).toEqual(["history A"]);
    expect(servstationMessagesForConversation([historyA, historyB], historyB.id).map((message) => message.text)).toEqual(["history B"]);
