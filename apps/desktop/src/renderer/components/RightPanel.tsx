...
import { ApiOutlined, CompressOutlined, FolderOpenOutlined, PlusOutlined, ReloadOutlined, StopOutlined, ToolOutlined } from "@ant-design/icons";
import { Button, Divider, Empty, Input, Popconfirm, Space, Switch, Tabs, Tag, message } from "antd";
import { defaultServstationBaseUrl, defaultServstationClientId, defaultServstationIssuerUrl, defaultServstationRedirectUri, defaultServstationScope, defaultServstationUser, type AgentJob, type RemoteBridgeConfig, type RuntimeSnapshot, type ServstationA2AConfigUpdate } from "@supbot/shared";
import { formatDateTime, formatSchedule, statusColor, statusLabel } from "@supbot/shared";
import { AutopilotPanel } from "./AutopilotPanel";
import { MemoryPanel } from "./MemoryPanel";
import { assistantPreviewForJob, formatToolOutput, formatToolPayload, jobRuntimeEventColor, jobRuntimeEventLabel, toolStatusColor, truncateText } from "../lib/chatFormat";
...
export function JobExecutionTimeline({ snapshot, job, t }: {
  snapshot: RuntimeSnapshot;
  job: AgentJob;
  t: (key: string, vars?: Record<string, string | number>) => string;
}) {
...
export function WorktreesPanel({ snapshot, t }: { snapshot: RuntimeSnapshot; t: (key: string, vars?: Record<string, string | number>) => string }) {
...
export function RemoteBridgePanel({ snapshot, t }: { snapshot: RuntimeSnapshot; t: (key: string, vars?: Record<string, string | number>) => string }) {
...
export function RuntimeStatusPanel({ snapshot, t }: { snapshot: RuntimeSnapshot; t: (key: string, vars?: Record<string, string | number>) => string }) {
...
export function SchedulePanel({ snapshot, openSchedule, refresh, t }: { snapshot: RuntimeSnapshot; openSchedule: () => void; refresh: () => void; t: (key: string, vars?: Record<string, string | number>) => string }) {
  return (
    <div className="activity-list">
