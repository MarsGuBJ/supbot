...
export function ServerAgentMailWorkspace({
  ...
  const refreshMessages = useCallback(
    async (nextFolder: ServstationMessageFolder = folder) => {
      if (!connected) {
        setItems([]);
        setSelected(null);
        setUnreadCount(0);
  ...
  const openCompose = () => {
    const defaultAccount =
      accounts.find((account) => account.enabled && account.isDefault) || accounts.find((account) => account.enabled);
        ...
        <div className="server-agent-mail-list">
          {items.length ? (
            ...
            <div className="server-agent-mail-empty">
              {connected ? t("No messages") : t("Servstation reverse A2A is not connected.")}
            </div>
          )}
        </div>
      </section>
          ...
          <>
              ...
              <Space wrap>
                <Tooltip title={selected.favorited ? t("Unfavorite") : t("Favorite")}>
                  <Button
                    ...
                    onClick={() =>
                      void window.supbot
                        .setServstationMessageFavorite(selected.messageId, !selected.favorited)
                        .then((messageDetail) => {
                ...
                {folder === "trash" ? (
                  <>
                    <Button
                      ...
                      onClick={() =>
                        void window.supbot
                          .restoreServstationMessage(selected.messageId)
                          .then(() => {
                            setSelected(null);
                    ...
                    <Popconfirm
                      ...
                      onConfirm={() =>
                        void window.supbot
                          .deleteServstationMessage(selected.messageId)
                          .then(() => {
                            setSelected(null);
                            return refreshMessages("trash");
                          })
                    ...
                    >
                      <Button disabled={disabled} danger icon={<DeleteOutlined />}>
                        {t("Delete forever")}
                      </Button>
                    </Popconfirm>
                  </>
                  ...
                  <Button
                    ...
                    onClick={() =>
                      void window.supbot
                        .trashServstationMessage(selected.messageId)
                        .then(() => {
                          setSelected(null);
            ...
            <Descriptions size="small" column={1}>
              ...
              <Descriptions.Item label={t("To")}>
                {(selected.recipients || []).map(formatServstationAccountRef).join(", ") || "-"}
              </Descriptions.Item>
            </Descriptions>
            ...
            <div className="server-agent-mail-attachments">
              {(selected.attachments || []).map((attachment) => (
                <Button
                  ...
                  onClick={() =>
                    void window.supbot
                      .fetchServstationMessageAttachment(selected.messageId, attachment.attachmentId)
  ...
  const renderAccounts = () => (
    <section className="server-agent-mail-account-panel">
      <div className="server-agent-mail-panel-head">
        <Space wrap>
          <Button type="primary" icon={<PlusOutlined />} disabled={disabled} onClick={openAccountCreate}>
            {t("Add mail account")}
          </Button>
          <Button
            icon={<ReloadOutlined />}
      ...
      {accounts.length ? (
          ...
          {accounts.map((account) => (
            <article key={account.id} className="server-agent-mail-account-card">
              ...
              <div className="server-agent-mail-account-meta">
                ...
                <span>
                  {t("Last sync")}: {formatMessageTime(account.lastSyncAt)}
                </span>
                {account.lastSyncError ? <Alert type="warning" showIcon message={account.lastSyncError} /> : null}
              </div>
              <Space wrap>
                <Button icon={<EditOutlined />} disabled={disabled} onClick={() => openAccountEdit(account)}>
                  {t("Edit")}
                </Button>
                <Button
                  ...
                  onClick={() =>
                    void window.supbot
                      .setDefaultServstationMailAccount(account.id)
                      .then(() => {
                ...
                <Button
                  ...
                  onClick={() =>
                    void window.supbot
                      .syncServstationMailAccountNow(account.id)
                ...
                <Button
                  ...
                  onClick={() =>
                    void window.supbot
                      .testServstationMailAccountConnection(account.id)
                      .then((result) => {
                        const ok = result.imapOk && result.smtpOk;
                        messageApi[ok ? "success" : "warning"](
                          ok ? t("Connection test passed.") : t("Connection test failed."),
                        );
                        Modal.info({
                          ...
                          content: (
                            <div className="server-agent-mail-test-result">
                              <div>
                                SMTP: {String(result.smtpOk)}, IMAP: {String(result.imapOk)}
                              </div>
                ...
                <Popconfirm
                  ...
                  onConfirm={() =>
                    void window.supbot
                      .deleteServstationMailAccount(account.id)
                      .then(() => {
        ...
        <div className="server-agent-mail-empty">
          {connected ? t("No mail accounts") : t("Servstation reverse A2A is not connected.")}
        </div>
      )}
    </section>
  );
    ...
    <section className="server-agent-mail-workspace">
      ...
      <div className="server-agent-mail-toolbar">
        ...
        <Space wrap>
          ...
          <Button type="primary" icon={<SendOutlined />} disabled={disabled || !identity} onClick={openCompose}>
            {t("Send")}
          </Button>
        </Space>
      </div>
      ...
      <Modal
        ...
        destroyOnClose
        footer={[
          <Button key="cancel" onClick={() => setComposeOpen(false)}>
            {t("Cancel")}
          </Button>,
          <Button
            key="submit"
      ...
      >
        <Form form={composeForm} layout="vertical">
          ...
          <Form.Item label={t("Send as")} name="senderMailAccountId">
            <Select
              allowClear
              placeholder={t("No outbound mail account selected")}
          ...
          >
            <Upload beforeUpload={() => false} multiple disabled={disabled || sending}>
              <Button icon={<PaperClipOutlined />} disabled={disabled || sending}>
                {t("Attach files")}
              </Button>
            </Upload>
          </Form.Item>
      ...
      <Modal
        ...
        destroyOnClose
        footer={[
          <Button key="cancel" onClick={() => setAccountModalOpen(false)}>
            {t("Cancel")}
          </Button>,
          <Button key="submit" type="primary" loading={savingAccount} onClick={() => void submitAccount()}>
            {t("Save")}
          </Button>,
        ]}
      >
        <Form form={accountForm} layout="vertical" className="server-agent-mail-account-form">
          <Form.Item
            ...
            name="emailAddress"
            rules={[{ required: true, type: "email", message: t("Email address") }]}
          >
          ...
          <div className="server-agent-mail-form-grid">
            ...
            <Form.Item
              ...
              name="smtpSecurity"
              rules={[{ required: true, message: t("SMTP security") }]}
            >
            ...
            <Form.Item
              ...
              name="smtpUsername"
              rules={[{ required: true, message: t("SMTP username") }]}
            >
              <Input disabled={disabled || savingAccount} />
            </Form.Item>
            <Form.Item
              ...
              name="smtpPassword"
              rules={[{ required: !editingAccount, message: t("SMTP password") }]}
            >
            ...
            <Form.Item
              ...
              name="imapSecurity"
              rules={[{ required: true, message: t("IMAP security") }]}
            >
              <Select options={servstationMailSecurityOptions(t)} disabled={disabled || savingAccount} />
            </Form.Item>
            <Form.Item
              ...
              name="imapUsername"
              rules={[{ required: true, message: t("IMAP username") }]}
            >
              <Input disabled={disabled || savingAccount} />
            </Form.Item>
            <Form.Item
              ...
              name="imapPassword"
              rules={[{ required: !editingAccount, message: t("IMAP password") }]}
            >
              <Input.Password autoComplete="new-password" disabled={disabled || savingAccount} />
            </Form.Item>
          </div>
...
export function normalizeServstationMailAccountDraft(
  values: ServerAgentMailAccountValues,
): ServstationMailAccountDraft {
...
export function fileToServstationMessageAttachment(
  file: Blob & { name: string; type?: string },
): Promise<ServstationMessageAttachmentUpload> {
