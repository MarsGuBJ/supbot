...
import { ApiOutlined, AppstoreOutlined, CheckCircleOutlined, DeleteOutlined, EditOutlined, PaperClipOutlined, PlusOutlined, ReloadOutlined, SaveOutlined, SettingOutlined, ThunderboltOutlined, ToolOutlined } from "@ant-design/icons";
import { Alert, Button, Divider, Form, Input, InputNumber, List, Modal, Popconfirm, Segmented, Select, Slider, Space, Switch, Tabs, Tag, Tooltip, Typography, message } from "antd";
import { defaultServstationUser, defaultToolMarketApiUrl, type CapabilityUpdateInput, type ModelProviderConfig, type ModelProviderUpdate, type PermissionRule, type PersonalityConfig, type RuntimeSnapshot, type ServstationA2AConfigUpdate, type SubagentConfig, type ToolMarketConfigUpdate } from "@supbot/shared";
...
export function MemorySettingsCard({ snapshot, refresh, t }: {
...
export function StorageCard({ userDataPath, t }: { userDataPath: string; t: (key: string, vars?: Record<string, string | number>) => string }) {
  return (
    <div className="settings-card">
...
export function ToolMarketConfigCard({ snapshot, refresh, t }: {
  snapshot: RuntimeSnapshot;
  refresh: () => void;
  t: (key: string, vars?: Record<string, string | number>) => string;
}) {
...
export function RemoteStaffAgentConfigCard({ snapshot, refresh, t }: {
  snapshot: RuntimeSnapshot;
  refresh: () => void;
  t: (key: string, vars?: Record<string, string | number>) => string;
}) {
...
export function ModelConfigCard({ snapshot, refresh, t }: { snapshot: RuntimeSnapshot; refresh: () => void; t: (key: string, vars?: Record<string, string | number>) => string }) {
              ...
              <List.Item.Meta
                title={(
                  <Space wrap>
                    ...
                    <Tag color={provider.apiKeySaved ? (provider.apiKeyStorage === "file" ? "orange" : "blue") : "default"}>
...
export function PersonalityCard({ snapshot, refresh, t }: { snapshot: RuntimeSnapshot; refresh: () => void; t: (key: string, vars?: Record<string, string | number>) => string }) {
...
export function CapabilitiesCard({ snapshot, refresh, t }: { snapshot: RuntimeSnapshot; refresh: () => void; t: (key: string, vars?: Record<string, string | number>) => string }) {
...
export function SubagentsCard({ snapshot, refresh, openSubagent, t }: { snapshot: RuntimeSnapshot; refresh: () => void; openSubagent: (subagent: SubagentConfig | null) => void; t: (key: string, vars?: Record<string, string | number>) => string }) {
  return (
    <div className="settings-card">
      <div className="panel-heading">
...
export function filterVisibleCapabilities(capabilities: RuntimeSnapshot["capabilities"] | undefined): RuntimeSnapshot["capabilities"] {
