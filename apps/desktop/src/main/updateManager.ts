...
export class HBClientUpdateManager {
  ...
  private async withAuthenticatedFeed<T>(action: () => Promise<T>): Promise<T> {
    ...
    throw new Error("HyBotLeasing update authorization failed.");
  }
...
function requireUpdatePlatform(): HBClientUpdatePlatform {
  ...
  if (!platform) {
    throw new Error(`HyBotLeasing updates are not supported on ${process.platform}-${process.arch}.`);
  }
  return platform;
}
...
export function requireHttpsUpdateFeedUrl(value: string): string {
  ...
  try {
    ...
    throw new Error("HyBotLeasing update feed URL is invalid.", { cause: error });
  }
  if (url.protocol !== "https:") {
    throw new Error("HyBotLeasing update feed requires HTTPS; refusing an unencrypted update channel.");
  }
  url.username = "";
  url.password = "";
