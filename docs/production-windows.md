...
HyWork checks the authenticated Botstation stable update feed after connecting and every 30 minutes. It never downloads or installs an update without user action.

Publish a release from Botstation Admin Console:

1. Build and sign the Windows x64 NSIS installer.
2. Confirm the file name is `HyWork-{version}-win-x64.exe` and the embedded application version uses the same SemVer value.
3. Open **客户端版本**, upload the EXE, confirm the version, and publish it.
4. Verify an installed older HyWork displays the upgrade prompt, downloads the package, and offers **重启并安装**.
