...
> {
  ...
  while (true) {
        ...
        const content =
          retryDelayMs !== undefined && retryDelayMs > 0
            ? 'The conversation is over the auto-compact threshold, but automatic compaction is cooling down after repeated failures. ' +
              'KennyHarness stopped before sending another oversized request. ' +
            ...
            : 'The conversation is over the auto-compact threshold and automatic compaction has failed repeatedly. ' +
              'KennyHarness stopped before sending another oversized request. Run /compact, undo recent large tool output, or start a new session with /new.'
        yield createAssistantAPIErrorMessage({
          content,
          error: 'invalid_request',
        })
    ...
    // These will be sent as attachments so KennyHarness can respond to them in the current turn.
    //
