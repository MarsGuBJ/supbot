...
const envKeys = [
  ...
  'KENNYHARNESS_MAX_RETRIES',
  'KENNYHARNESS_RETRY_DELAY_MS',
  'OPENAI_MODEL',
  'OPENAI_BASE_URL',
  'OPENAI_API_BASE',
] as const
...
describe('retry configuration', () => {
  ...
  test('reads retry attempts from KENNYHARNESS_MAX_RETRIES', async () => {
    process.env.KENNYHARNESS_MAX_RETRIES = '4'
    const { getDefaultMaxRetries } = await importFreshWithRetryModule()
    expect(getDefaultMaxRetries()).toBe(4)
  })

  test('allows zero retry attempts', async () => {
    process.env.KENNYHARNESS_MAX_RETRIES = '0'
    const { getDefaultMaxRetries } = await importFreshWithRetryModule()
  ...
  test('prefers KENNYHARNESS_MAX_RETRIES over legacy CLAUDE_CODE_MAX_RETRIES', async () => {
    process.env.KENNYHARNESS_MAX_RETRIES = '3'
    process.env.CLAUDE_CODE_MAX_RETRIES = '0'
    const { getDefaultMaxRetries } = await importFreshWithRetryModule()
  ...
  test('falls back to default retry attempts for invalid values', async () => {
    process.env.KENNYHARNESS_MAX_RETRIES = 'nope'
    const { getDefaultMaxRetries } = await importFreshWithRetryModule()
    expect(getDefaultMaxRetries()).toBe(10)
  })

  test('caps retry attempts to a bounded value', async () => {
    process.env.KENNYHARNESS_MAX_RETRIES = '1000'
    const { getDefaultMaxRetries } = await importFreshWithRetryModule()
  ...
  test('reads retry delay from KENNYHARNESS_RETRY_DELAY_MS', async () => {
    process.env.KENNYHARNESS_RETRY_DELAY_MS = '1500'
    const { getDefaultRetryDelayMs } = await importFreshWithRetryModule()
    expect(getDefaultRetryDelayMs()).toBe(1500)
  })

  test('falls back to default retry delay for invalid values', async () => {
    process.env.KENNYHARNESS_RETRY_DELAY_MS = '-1'
    const { getDefaultRetryDelayMs } = await importFreshWithRetryModule()
    expect(getDefaultRetryDelayMs()).toBe(500)
  })

  test('uses configured retry delay as exponential backoff base', async () => {
    process.env.KENNYHARNESS_RETRY_DELAY_MS = '2000'
