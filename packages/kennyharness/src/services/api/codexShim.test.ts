...
function createTempAuthJson(payload: Record<string, unknown>): string {
  const dir = mkdtempSync(join(tmpdir(), 'kennyharness-codex-'))
  tempDirs.push(dir)
...
describe('Codex request translation', () => {
  ...
  test('recovers Codex web search text and sources from sparse completed response', () => {
    const output = webSearchToolTest.makeOutputFromCodexWebSearchResponse(
        ...
        output: [
          {
            ...
            sources: [
              {
                title: 'KennyHarness repo',
                url: 'https://github.com/example/kennyharness',
              },
            ],
          },
          {
            ...
            content: [
              {
                type: 'text',
                text: 'KennyHarness is available on GitHub.',
                sources: [
                  {
                    title: 'Docs',
                    url: 'https://docs.example.com/kennyharness',
                  },
                ],
              },
            ],
      ...
      'KennyHarness GitHub 2026',
      0.42,
    )

    expect(output.results).toEqual([
      'KennyHarness is available on GitHub.',
      {
            ...
            content: [
              {
                ...
                sources: [
                  { title: 'Docs', url: 'https://docs.example.com/kennyharness' },
                ],
              },
            ],
          },
  ...
  test('Codex stream: tool args delivered only via function_call_arguments.done (#1259)', async () => {
    const args = '{"path":"./kennyharness-codex-repro","pattern":"**/*.md"}'
    const responseText = [
      'event: response.output_item.added',
