...
export const AUTO_MODE_DESCRIPTION = "Auto mode lets KennyHarness handle permission prompts automatically — KennyHarness checks each tool call for risky actions and prompt injection before executing. Actions KennyHarness identifies as safe are executed, while actions KennyHarness identifies as risky are blocked and KennyHarness may try a different approach. Ideal for long-running tasks. Sessions are slightly more expensive. KennyHarness can make mistakes that allow harmful commands to run, it's recommended to only use in isolated environments. Shift+Tab to change mode.";
type Props = {
  onAccept(): void;
  onDecline(): void;
