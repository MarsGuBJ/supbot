...
export function ResumeTask({
  ...
  if (loading) {
    return <Box flexDirection="column" padding={1}>
        <Box flexDirection="row">
          <Spinner />
          <Text bold>Loading KennyHarness sessions…</Text>
        </Box>
        <Text dimColor>
          {retrying ? 'Retrying…' : 'Fetching your KennyHarness sessions…'}
        </Text>
      </Box>;
  }
  if (loadErrorType) {
    return <Box flexDirection="column" padding={1}>
        <Text bold color="error">
          Error loading KennyHarness sessions
        </Text>
  ...
  if (sessions.length === 0) {
    return <Box flexDirection="column" padding={1}>
        <Text bold>
          No KennyHarness sessions found
...
function renderErrorSpecificGuidance(errorType: LoadErrorType): React.ReactNode {
  switch (errorType) {
    ...
    case 'api':
      return <Box marginY={1} flexDirection="column">
          <Text dimColor>Sorry, KennyHarness encountered an error</Text>
        </Box>;
    case 'other':
