 ...
 * return @path refs to prepend. KennyHarness's Read tool takes it from there.
 *
 * Best-effort: any failure (no token, network, non-2xx, disk) logs debug and
 * skips that attachment. The message still reaches KennyHarness, just without @path.
 */
