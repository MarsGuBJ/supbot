...
const originalEnv = {
  ...
  KENNYHARNESS_DISABLE_CO_AUTHORED_BY:
    process.env.KENNYHARNESS_DISABLE_CO_AUTHORED_BY,
...
const defaultPrAttribution =
  '🤖 Generated with [KennyHarness](https://github.com/Gitlawb/kennyharness)'
...
beforeEach(async () => {
  ...
  delete process.env.KENNYHARNESS_DISABLE_CO_AUTHORED_BY
...
describe('getDefaultCommitCoAuthorName', () => {
  ...
  it('uses the KennyHarness email for commit attribution across providers', () => {
    expect(getDefaultCommitCoAuthorEmail('openai')).toBe(
      'kennyharness@gitlawb.com',
    )
...
describe('getAttributionTexts', () => {
  ...
  it('preserves includeCoAuthoredBy true as an explicit old-default opt-in', () => {
    ...
    expect(getAttributionTexts()).toEqual({
      commit: 'Co-Authored-By: KennyHarness (gpt-5.5) <kennyharness@gitlawb.com>',
  ...
  it('uses KENNYHARNESS_DISABLE_CO_AUTHORED_BY to disable the old default co-author trailer', () => {
    process.env.KENNYHARNESS_DISABLE_CO_AUTHORED_BY = '1'
    useSettings({ includeCoAuthoredBy: true })
  ...
  it('does not let KENNYHARNESS_DISABLE_CO_AUTHORED_BY override explicit commit attribution', () => {
    process.env.KENNYHARNESS_DISABLE_CO_AUTHORED_BY = '1'
    useSettings({
