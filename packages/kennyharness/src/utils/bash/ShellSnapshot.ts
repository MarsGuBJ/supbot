...
/**
 * Generates KennyHarness specific snapshot content
...
RIPGREP_FUNC_END
  ...
  // consistent fast behavior in KennyHarness's shell.
...
async function getSnapshotScript(
  ...
  // Generate the user content and KennyHarness content
 ...
 *
 * This function is a critical part of KennyHarness's shell integration strategy. It:
 *
 ...
 *
 * This approach allows KennyHarness to execute commands as if they were run in the user's
...
export const createAndSaveSnapshot = async (
  ...
  return new Promise(async resolve => {
    try {
      ...
      if (!configFileExists) {
        logForDebugging(
          `Shell config file not found: ${configFile}, creating snapshot with KennyHarness defaults only`,
        )
      }
      ...
      execFile(
        ...
        async (error, stdout, stderr) => {
          if (error) {
            ...
            logForDebugging(`  - Config home: ${getClaudeConfigHomeDir()}`)
