...
beforeEach(async () => {
  ...
  tempDir = mkdtempSync(join(tmpdir(), 'kennyharness-env-test-'))
...
test('getGlobalClaudeFile: new install returns .kennyharness.json when neither file exists', async () => {
  const { getGlobalClaudeFile } = await importFreshEnvModule()
  expect(getGlobalClaudeFile()).toBe(join(tempDir, '.kennyharness.json'))
})
...
test('getGlobalClaudeFile: migrated user uses .kennyharness.json when both files exist', async () => {
  writeFileSync(join(tempDir, '.claude.json'), '{}')
  writeFileSync(join(tempDir, '.kennyharness.json'), '{}')
  const { getGlobalClaudeFile } = await importFreshEnvModule()
  expect(getGlobalClaudeFile()).toBe(join(tempDir, '.kennyharness.json'))
})
