...
const PROJECT_CONFIG_DIR_NAMES = ['.claude', '.kennyharness'] as const
...
// ~/.kennyharness/agents — see issue #769) opens that many fds and blocks the
...
function recordOversizedSkip(skip: OversizedMarkdownSkip): void {
  ...
  if (!oversizedSkipStderrWarned) {
    ...
    process.stderr.write(
      `kennyharness: skipping oversized markdown config file ${skip.filePath} ` +
