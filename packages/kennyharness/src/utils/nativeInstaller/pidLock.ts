 ...
 *
 * This module provides PID-based locking for running KennyHarness versions.
...
/**
 * Validate that a running process is actually a KennyHarness process
...
export function isLockActive(lockFilePath: string): boolean {
  ...
  // Secondary validation: is it actually a KennyHarness process?
  // This helps with PID reuse scenarios
  if (!isClaudeProcess(pid, execPath)) {
    logForDebugging(
      `Lock PID ${pid} is running but does not appear to be KennyHarness - treating as stale`,
    )
    return false
  }
...
export async function tryAcquireLock(
  ...
  // validates it's actually a KennyHarness process (to handle PID reuse scenarios)
