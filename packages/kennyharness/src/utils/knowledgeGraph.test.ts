...
describe('KnowledgeGraph Global Persistence & RAG', () => {
  ...
  beforeEach(async () => {
    ...
    configDir = mkdtempSync(join(tmpdir(), 'kennyharness-test-'))
  ...
  it('persists entities across loads', async () => {
    await addGlobalEntity('tool', 'kennyharness', { status: 'alpha' })
    ...
    const entities = Object.values(graph.entities).filter(e => e.name === 'kennyharness')
  ...
  it('deduplicates entities and updates attributes', async () => {
    await addGlobalEntity('tool', 'kennyharness', { status: 'alpha' })
    await addGlobalEntity('tool', 'kennyharness', { status: 'beta', version: '0.6.0' })
