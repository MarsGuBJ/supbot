...
describe('KnowledgeGraph Phase 1 Stress & Edge Cases', () => {
  ...
  const originalOrama = process.env.KENNYHARNESS_KNOWLEDGE_ORAMA
  const configDir = mkdtempSync(join(tmpdir(), 'kennyharness-stress-'))
  ...
  beforeEach(async () => {
    ...
    process.env.KENNYHARNESS_KNOWLEDGE_ORAMA = '1'
  ...
  afterEach(() => {
    try {
      ...
      if (originalOrama === undefined) {
        delete process.env.KENNYHARNESS_KNOWLEDGE_ORAMA
      } else {
        process.env.KENNYHARNESS_KNOWLEDGE_ORAMA = originalOrama
      }
