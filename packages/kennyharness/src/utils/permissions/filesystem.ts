 ...
 * .kennyharness/skills/{name}/ directory (global), plus the legacy global
...
export function getClaudeSkillScope(
  ...
  const bases = [
    ...
    {
      dir: expandPath(join(homedir(), '.kennyharness', 'skills')),
      prefix: '~/.kennyharness/skills/',
    },
    {
...
export function isClaudeSettingsPath(filePath: string): boolean {
  ...
  if (
    normalizedPath.endsWith(`${sep}.kennyharness${sep}settings.json`) ||
    normalizedPath.endsWith(`${sep}.kennyharness${sep}settings.local.json`) ||
...
function isClaudeConfigFilePath(filePath: string): boolean {
  ...
  const openCommandsDir = join(getOriginalCwd(), '.kennyharness', 'commands')
  const openAgentsDir = join(getOriginalCwd(), '.kennyharness', 'agents')
  const openSkillsDir = join(getOriginalCwd(), '.kennyharness', 'skills')
...
export function isKennyHarnessCommitMessagePath(absolutePath: string): boolean {
  const expectedPath = join(getOriginalCwd(), '.git', 'KENNYHARNESS_COMMIT_MSG')
...
export function checkEditableInternalPath(
  ...
  if (
    ...
    isKennyHarnessCommitMessagePath(normalizedPath)
  ) {
    return {
      behavior: 'allow',
      updatedInput: input,
      decisionReason: {
        type: 'other',
        reason: 'KennyHarness commit message file is allowed for writing',
      },
    }
  }
