...
describe('KennyHarness commit message temp file permissions', () => {
  let originalCwd: string
  let projectDir: string

  beforeEach(async () => {
    originalCwd = getOriginalCwd()
    projectDir = await mkdtemp(join(tmpdir(), 'kennyharness-perms-'))
  ...
  test('allows the project-local KENNYHARNESS_COMMIT_MSG file without a safety prompt', () => {
    const result = checkWritePermissionForTool(
      writeTool,
      { file_path: join(projectDir, '.git', 'KENNYHARNESS_COMMIT_MSG') },
  ...
  test('allows the project-local KENNYHARNESS_COMMIT_MSG file in fullAccess mode', () => {
    const result = checkWritePermissionForTool(
      writeTool,
      { file_path: join(projectDir, '.git', 'KENNYHARNESS_COMMIT_MSG') },
  ...
  test('does not allow same-named files outside the project git directory', () => {
    ...
    const result = checkWritePermissionForTool(
      writeTool,
      { file_path: join(otherDir, '.git', 'KENNYHARNESS_COMMIT_MSG') },
      permissionContext('bypassPermissions'),
    )
