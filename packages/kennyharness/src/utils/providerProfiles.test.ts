...
beforeEach(async () => {
  ...
  testConfigDir = mkdtempSync(join(tmpdir(), 'kennyharness-provider-config-'))
...
describe('setActiveProviderProfile', () => {
  test('sets OPENAI_MODEL env var when switching to an openai-type provider', async () => {
    const configDir = mkdtempSync(join(tmpdir(), 'kennyharness-provider-config-'))
  ...
  test('persists xAI OAuth profile with marker so logout cleanup can clear it', async () => {
    ...
    try {
      ...
      const profilePath = join(configDir, '.kennyharness-profile.json')
  ...
  test('persists no-key openai-compatible profiles for restart fallback', async () => {
    ...
    try {
      ...
      const persisted = JSON.parse(
        readFileSync(join(configDir, '.kennyharness-profile.json'), 'utf8'),
      )

      expect(result?.id).toBe('ollama_prof')
      expect(existsSync(join(tempDir, '.kennyharness-profile.json'))).toBe(false)
