 ...
 * =====================
 * This module manages an isolated tmux socket for KennyHarness's operations.
 *
 * WHY THIS EXISTS:
 * Without isolation, KennyHarness could accidentally affect the user's tmux sessions.
 * For example, running `tmux kill-session` via the Bash tool would kill the
 * user's current session if they started KennyHarness from within tmux.
 *
 * HOW IT WORKS:
 * 1. KennyHarness creates its own tmux socket: `claude-<PID>` (e.g., `claude-12345`)
 ...
 *
 * This means ANY tmux command run through KennyHarness - whether via the Tmux tool
 * directly or via Bash - will operate on KennyHarness's isolated socket, NOT the
 * user's tmux session.
 *
...
/**
 * Gets the socket name for KennyHarness's isolated tmux session.
 * Format: claude-<PID>
 */
...
/**
 * Gets the TMUX environment variable value for KennyHarness's isolated socket.
 *
 ...
 * the Bash tool will operate on KennyHarness's socket, NOT the user's session.
 *
...
/**
 * Kills the tmux server for KennyHarness's isolated socket.
...
async function doInitialize(): Promise<void> {
  ...
  // Any KennyHarness instance spawned on this socket will inherit this env var,
