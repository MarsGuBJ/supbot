...
beforeEach(async () => {
  await acquireSharedMutationLock('utils/kennyharnessInstallSurfaces.test.ts')
})
...
test('install command displays ~/.local/bin/kennyharness on non-Windows', async () => {
  ...
  expect(getInstallationPath()).toBe('~/.local/bin/kennyharness')
})

test('install command displays kennyharness.exe path on Windows', async () => {
  mock.module('../utils/env.js', () => ({
    ...realEnv,
    env: { platform: 'win32' },
  }))
  ...
  expect(getInstallationPath()).toBe(
    join(homedir(), '.local', 'bin', 'kennyharness.exe').replace(/\//g, '\\'),
  )
})

test('cleanupNpmInstallations removes both kennyharness and legacy claude local install dirs', async () => {
  ...
  mock.module('./envUtils.js', () => ({
    ...realEnvUtils,
    getClaudeConfigHomeDir: () => join(homedir(), '.kennyharness'),
  ...
  expect(removedPaths).toContain(join(homedir(), '.kennyharness', 'local'))
