      ...
      case 'macos': {
        ...
        try {
          ...
          if (stats.isDirectory()) {
            logForDebugging(
              `[Browser control] Detected browser: ${config.name}`,
            )
            return browserId
          }
        } catch (e) {
...
export async function openInChrome(url: string): Promise<boolean> {
  ...
  if (!browser) {
    logForDebugging('[Browser control] No compatible browser found')
    return false
  }
...
function getSocketName(): string {
  // NOTE: This must match the one used in the browser bridge MCP server
