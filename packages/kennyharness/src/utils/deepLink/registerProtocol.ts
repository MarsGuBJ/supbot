 ...
 * KennyHarness then uses the url-handler NAPI module to read the URL from the
...
<dict>
  ...
  <array>
    <dict>
      ...
      <string>KennyHarness Deep Link</string>
...
Name=${APP_NAME}
Comment=Handle ${DEEP_LINK_PROTOCOL}:// deep links for KennyHarness
 ...
 * a cached flag in ~/.kennyharness.json, so:
...
export async function ensureDeepLinkProtocolRegistered(): Promise<void> {
  ...
  // ~/.claude (per-machine, not synced) rather than ~/.kennyharness.json (can sync).
