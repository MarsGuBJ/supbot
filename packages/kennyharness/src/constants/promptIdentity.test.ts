...
beforeAll(async () => {
  ...
  ;(globalThis as Record<string, unknown>).MACRO = {
    ...
    ISSUES_EXPLAINER:
      'report the issue at https://github.com/Gitlawb/kennyharness/issues',
    PACKAGE_URL: 'kennyharness',
...
test('CLI identity prefixes describe KennyHarness instead of Claude Code', () => {
  expect(getCLISyspromptPrefix()).toContain('KennyHarness')
  ...
  for (const prefix of CLI_SYSPROMPT_PREFIXES) {
    expect(prefix).toContain('KennyHarness')
    expect(prefix).not.toContain('Claude Code')
    expect(prefix).not.toContain("Anthropic's official CLI for Claude")
  }
})

test('simple mode identity describes KennyHarness instead of Claude Code', async () => {
  process.env.CLAUDE_CODE_SIMPLE = '1'

  const prompt = await getSystemPrompt([], 'gpt-4o')

  expect(prompt[0]).toContain('KennyHarness')
...
test('built-in agent prompts describe KennyHarness instead of Claude Code', () => {
  expect(DEFAULT_AGENT_PROMPT).toContain('KennyHarness')
  ...
  expect(generalPrompt).toContain('KennyHarness')
  ...
  expect(explorePrompt).toContain('KennyHarness')
  ...
  expect(guidePrompt).toContain('KennyHarness')
  expect(guidePrompt).toContain('You are the KennyHarness guide agent.')
  expect(guidePrompt).toContain('**KennyHarness** (the CLI tool)')
