...
Manage the KennyHarness project wiki stored in .kennyharness/wiki.
...
function formatInitResult(result: Awaited<ReturnType<typeof initializeWiki>>): string {
  const lines = [`Initialized KennyHarness wiki at ${result.root}`]
...
function formatStatus(status: Awaited<ReturnType<typeof getWikiStatus>>): string {
  if (!status.initialized) {
    return `KennyHarness wiki is not initialized in this project.\n\nRun /wiki init to create ${status.root}.`
  }

  return [
    'KennyHarness wiki status',
    '',
    `Root: ${status.root}`,
...
function formatIngestResult(
  ...
  return [
    `Ingested ${result.sourceFile} into the KennyHarness wiki.`,
    '',
    `Title: ${result.title}`,
