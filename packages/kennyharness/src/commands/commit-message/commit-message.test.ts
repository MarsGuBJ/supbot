...
describe('commit-message command helpers', () => {
  it('parses quoted co-author names with a plain email', () => {
    expect(parseCoAuthor('"GPT 5.5" noreply@kennyharness.dev')).toEqual({
      name: 'GPT 5.5',
      email: 'noreply@kennyharness.dev',
    })
  })

  it('parses co-author trailers with angle-bracket emails', () => {
    expect(parseCoAuthor('KennyHarness (gpt-5.5) <noreply@kennyharness.dev>')).toEqual(
      {
        name: 'KennyHarness (gpt-5.5)',
        email: 'noreply@kennyharness.dev',
      },
    )
  })
  ...
  it('rejects co-author trailers with empty sanitized names', () => {
    expect(parseCoAuthor('"  " noreply@kennyharness.dev')).toBeNull()
    expect(parseCoAuthor('"  " <noreply@kennyharness.dev>')).toBeNull()
  })

  it('strips one pair of matching quotes from custom attribution text', () => {
    expect(stripMatchingQuotes('"Generated with KennyHarness"')).toBe(
      'Generated with KennyHarness',
    )
    expect(stripMatchingQuotes("'Generated with KennyHarness'")).toBe(
      'Generated with KennyHarness',
    )
  ...
  it('formats a sanitized co-author trailer', () => {
    expect(
      formatCoAuthorTrailer('KennyHarness <gpt>\n', '<noreply@kennyharness.dev>'),
    ).toBe('Co-Authored-By: KennyHarness gpt <noreply@kennyharness.dev>')
  })
  ...
  it('makes set scope explicit with example text', () => {
    ...
    expect(USAGE).toContain(
      '/commit-message set "Generated with KennyHarness using GPT-5.5"',
    )
    expect(USAGE).not.toContain('/commit-message set-attribution')
  })

  it('describes default reset as privacy-preserving', async () => {
    tempSettingsDir = mkdtempSync(join(tmpdir(), 'kennyharness-settings-'))
