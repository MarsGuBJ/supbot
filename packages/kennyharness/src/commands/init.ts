...
const OLD_INIT_PROMPT = `Please analyze this codebase and create a CLAUDE.md file, which will be given to future instances of KennyHarness to operate in this repository.
...
This file provides guidance to KennyHarness (claude.ai/code) when working with code in this repository.
\`\`\``

const NEW_INIT_PROMPT = `Set up a minimal AGENTS.md (and optionally CLAUDE.local.md, skills, and hooks) for this repo. The root project instruction file is loaded into every KennyHarness session, so it must be concise — only include what KennyHarness would get wrong without it.
...
- "Also set up skills and hooks?"
  ...
  Description for skills: "On-demand capabilities you or KennyHarness invoke with \`/skill-name\` — good for repeatable workflows and reference knowledge."
  Description for hooks: "Deterministic shell commands that run on tool events (e.g., format after every edit). KennyHarness can't skip them."
...
If the user chose personal CLAUDE.local.md or both: ask about them, not the codebase. Do not mark any options as "recommended" — this is about their personal preferences, not best practices. Examples of questions:
  ...
  - How familiar are they with this codebase and its languages/frameworks? (so KennyHarness can calibrate explanation depth)
  - Do they have personal sandbox URLs, test accounts, API key paths, or local setup details KennyHarness should know?
...
**Synthesize a proposal from Phase 2 findings** — e.g., format-on-edit if a formatter exists, a project verification workflow if tests exist, an AGENTS.md note for anything from the gap-fill answers that's a guideline rather than a workflow. For each, pick the artifact type that fits, **constrained by the Phase 1 skills+hooks choice**:
  ...
  - **Hook** (stricter) — deterministic shell command on a tool event; KennyHarness can't skip it. Fits mechanical, fast, per-edit steps: formatting, linting, running a quick test on the changed file.
  - **Skill** (on-demand) — you or KennyHarness invoke \`/skill-name\` when you want it. Fits workflows that don't belong on every edit: deep verification, session reports, deploys.
  - **AGENTS.md note** (looser) — influences KennyHarness's behavior but not enforced. Fits communication/thinking preferences: "plan before coding", "be terse", "explain tradeoffs".
...
Write a minimal AGENTS.md at the project root. Every line must pass this test: "Would removing this cause KennyHarness to make mistakes?" If no, cut it.
...
**Consume \`note\` entries from the Phase 3 preference queue whose target is AGENTS.md** (team-level notes) — add each as a concise line in the most relevant section. These are the behaviors the user wants KennyHarness to follow but didn't need guaranteed (e.g., "propose a plan before implementing", "explain the tradeoffs when refactoring"). Leave personal-targeted notes for Phase 5.

Include:
- Build/test/lint commands KennyHarness can't guess (non-standard scripts, flags, or sequences)
...
Exclude:
- File-by-file structure or component lists (KennyHarness can discover these by reading the codebase)
- Standard language conventions KennyHarness already knows
...
- Information that changes frequently — reference the source with \`@path/to/import\` so KennyHarness always reads the current version
...
For projects with distinct subdirectories (monorepos, multi-module projects, etc.): mention that subdirectory AGENTS.md files can be added for module-specific instructions (they're loaded automatically when KennyHarness works in those directories). Offer to create them if the user wants.
...
Include:
- The user's role and familiarity with the codebase (so KennyHarness can calibrate explanations)
- Personal sandbox URLs, test accounts, or local setup details
- Personal workflow or communication preferences

Keep it short — only include what would make KennyHarness's responses noticeably better for this user.
...
Skills add capabilities KennyHarness can use on demand without bloating every session.
...
<Instructions for KennyHarness>
\`\`\`

Both the user (\`/<skill-name>\`) and KennyHarness can invoke skills by default. For workflows with side effects (e.g., \`/deploy\`, \`/fix-issue 123\`), add \`disable-model-invocation: true\` so only the user can trigger it, and use \`$ARGUMENTS\` to accept input.
...
- **GitHub CLI**: Run \`which gh\` (or \`where gh\` on Windows). If it's missing AND the project uses GitHub (check \`git remote -v\` for github.com), ask the user if they want to install it. Explain that the GitHub CLI lets KennyHarness help with commits, pull requests, issues, and code review directly.

- **Linting**: If Phase 2 found no lint config (no .eslintrc, ruff.toml, .golangci.yml, etc. for the project's language), ask the user if they want KennyHarness to set up linting for this codebase. Explain that linting catches issues early and gives KennyHarness fast feedback on its own edits.
...
- **Proposal-sourced hooks** (if user chose "Skills + hooks" or "Hooks only"): Consume \`hook\` entries from the Phase 3 preference queue. If Phase 2 found a formatter and the queue has no formatting hook, offer format-on-edit as a fallback. If the user chose "Neither" or "Skills only" in Phase 1, skip this bullet entirely.
  ...
  2. Pick the event and matcher from the preference:
     ...
     - "when KennyHarness finishes" / "before I review" → \`Stop\` event (fires at the end of every turn — including read-only ones)
     - "before running bash" → \`PreToolUse\` with matcher \`Bash\`
     - "before committing" (literal git-commit gate) → **not a hooks.json hook.** Matchers can't filter Bash by command content, so there's no way to target only \`git commit\`. Route this to a git pre-commit hook (\`.git/hooks/pre-commit\`, husky, pre-commit framework) instead — offer to write one. If the user actually means "before I review and commit KennyHarness's output", that's \`Stop\` — probe to disambiguate.
...
Then tell the user that you'll be introducing a few more suggestions for optimizing their codebase and KennyHarness setup based on what you found. Present these as a single, well-formatted to-do list where every item is relevant to this repo. Put the most impactful items first.

When building the list, work through these checks and include only what applies:
- If frontend code was detected (React, Vue, Svelte, etc.): \`/plugin install frontend-design@claude-plugins-official\` gives KennyHarness design principles and component patterns so it produces polished UI; \`/plugin install playwright@claude-plugins-official\` lets KennyHarness launch a real browser, screenshot what it built, and fix visual bugs itself.
- If you found gaps in Phase 7 (missing GitHub CLI, missing linting) and the user said no: list them here with a one-line reason why each helps.
- If tests are missing or sparse: suggest setting up a test framework so KennyHarness can verify its own changes.
- To help you create skills and optimize existing skills using evals, KennyHarness has an official skill-creator plugin you can install. Install it with \`/plugin install skill-creator@claude-plugins-official\`, then run \`/skill-creator <skill-name>\` to create new skills or refine any existing skill. (Always include this one.)
