...
Hooks run commands at specific points in KennyHarness's lifecycle.
...
| Stop | - | Run when KennyHarness stops (including clear, resume, compact) |
...
Modify KennyHarness configuration by updating settings.json files.
...
User: "Format my code after KennyHarness writes it"
...
6. **Use --debug** - Run \`kennyharness --debug\` to see hook execution logs
`
...
export function registerUpdateConfigSkill(): void {
  registerBundledSkill({
    name: 'update-config',
    description:
      'Use this skill to configure the KennyHarness harness via settings.json. Automated behaviors ("from now on when X", "each time X", "whenever X", "before/after X") require hooks configured in settings.json - the harness executes these, not KennyHarness, so memory/preferences cannot fulfill them. Also use for: permissions ("allow X", "add permission", "move permission to"), env vars ("set X=Y"), hook troubleshooting, or any changes to settings.json/settings.local.json files. Examples: "allow npm commands", "add bq permission to global settings", "move permission to user settings", "set DEBUG=true", "when KennyHarness stops show X". For simple settings like theme/model, use Config tool.',
    allowedTools: ['Read'],
    userInvocable: true,
