...
import { isKennyHarnessCommitMessagePath } from '../../utils/permissions/filesystem.js'
import {
...
export function isUnsafeDotGitWritePathForPowerShell(
  ...
  if (
    ...
    isKennyHarnessCommitMessagePath(resolve(getOriginalCwd(), path))
  ) {
    return false
  }
...
export async function powershellToolHasPermission(
  ...
  for (const { text: subCmd, element, statement } of subCommands) {
    ...
    // .claude/settings.json (an agent config file the path validator didn't check).
