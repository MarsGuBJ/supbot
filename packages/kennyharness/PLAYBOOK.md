# KennyHarness Local Agent Playbook

This playbook is a practical guide to run KennyHarness with a local model (Ollama), work safely, and get strong day-to-day results.
...
If everything is healthy, KennyHarness starts directly.
...
Or let KennyHarness recommend the best local model for your goal:
...
```powershell
Get-Content .\.kennyharness-profile.json
```
...
- Keep any legacy workspace `.kennyharness-profile.json` local; new provider saves use the user config directory.
