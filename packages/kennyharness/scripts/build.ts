/**
 * KennyHarness build script — bundles the TypeScript source into a single
...
result = await Bun.build({
  ...
  define: {
    ...
    // version separately in KennyHarness branding.
    'MACRO.VERSION': JSON.stringify('99.0.0'),
    'MACRO.DISPLAY_VERSION': JSON.stringify(version),
    'MACRO.BUILD_TIME': JSON.stringify(new Date().toISOString()),
    'MACRO.ISSUES_EXPLAINER':
      JSON.stringify('report the issue at https://github.com/Gitlawb/kennyharness/issues'),
    'MACRO.FEEDBACK_CHANNEL':
      JSON.stringify('https://github.com/Gitlawb/kennyharness/issues'),
    'MACRO.PACKAGE_URL': JSON.stringify('kennyharness'),
...
if (!result.success) {
  ...
  console.log(`✓ Built kennyharness v${version} → dist/cli.mjs`)
}
