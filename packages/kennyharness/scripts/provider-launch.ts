...
function parseLaunchOptions(argv: string[]): LaunchOptions {
  ...
  let fast = false
  let goal = normalizeRecommendationGoal(process.env.KENNYHARNESS_PROFILE_GOAL)
...
async function main(): Promise<void> {
  ...
  const devCode = await runProcess('node', ['bin/kennyharness', ...options.passthroughArgs], env)
