...
const BIN_PATH = join(import.meta.dir, '..', 'bin', 'kennyharness')

describe('kennyharness launcher heap guard', () => {
  ...
  test('keeps user and troubleshooting escape hatches', () => {
    ...
    expect(source).toContain('KENNYHARNESS_DISABLE_HEAP_RELAUNCH')
    expect(source).toContain('KENNYHARNESS_NODE_MAX_OLD_SPACE_SIZE_MB')
