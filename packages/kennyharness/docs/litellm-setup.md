...
KennyHarness can connect to LiteLLM through LiteLLM's OpenAI-compatible proxy.

## Overview

LiteLLM is an open-source LLM gateway that provides a unified API to 100+ model providers. By running the LiteLLM Proxy, you can route KennyHarness requests through LiteLLM to access any of its supported providers — all while using KennyHarness's existing OpenAI-compatible provider path.
...
## 2. Point KennyHarness to LiteLLM
...
- LiteLLM's OpenAI-compatible endpoint accepts the same request format as OpenAI, so KennyHarness works without any code changes.
- You can switch between any provider configured in LiteLLM by simply changing the `OPENAI_MODEL` value — no need to reconfigure KennyHarness.
...
| Tools fail but chat works | The selected model has weak function/tool calling support | Switch to a model with strong tool support (e.g., GPT-4o, Anthropic's Sonnet models) |
