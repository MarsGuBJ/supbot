...
const { ChatController, KennyHarnessChatViewProvider, KennyHarnessChatPanelManager } = require('./chat/chatProvider');
const { SessionManager } = require('./chat/sessionManager');
const { DiffContentProvider, SCHEME: DIFF_SCHEME } = require('./chat/diffController');

const KENNYHARNESS_REPO_URL = 'https://github.com/Gitlawb/kennyharness';
const KENNYHARNESS_SETUP_URL = 'https://github.com/Gitlawb/kennyharness/blob/main/README.md#quick-start';
const PROFILE_FILE_NAME = '.kennyharness-profile.json';
...
async function collectControlCenterState() {
  const configured = vscode.workspace.getConfiguration('kennyharness');
  const launchCommand = configured.get('launchCommand', 'kennyharness');
  const terminalName = configured.get('terminalName', 'KennyHarness');
...
async function launchKennyHarness(options = {}) {
  ...
  if (!installed) {
    const action = await vscode.window.showErrorMessage(
      `KennyHarness command not found: ${executable}. Install it with: npm install -g kennyharness@latest`,
      'Open Setup Guide',
      'Open Repository',
    );

    if (action === 'Open Setup Guide') {
      await vscode.env.openExternal(vscode.Uri.parse(KENNYHARNESS_SETUP_URL));
    } else if (action === 'Open Repository') {
      await vscode.env.openExternal(vscode.Uri.parse(KENNYHARNESS_REPO_URL));
    }
...
function getWorkspaceRootActionDetail(status, fallbackDetail) {
  ...
  if (status.launchActionsShareTargetReason === 'relative-launch-command') {
    return `Same workspace-root target as Launch KennyHarness because the relative command resolves from the workspace root · ${status.workspaceRootCwdLabel}`;
  }
      ...
      <header class="hero">
        <div class="hero-top">
          <div class="brand">
            ...
            <div class="wordmark" aria-label="KennyHarness wordmark">Kenny<span class="wordmark-accent">Harness</span></div>
            <div class="headline">
        ...
        <section class="support-card" aria-labelledby="quick-links-title">
          ...
          <div class="support-stack">
            ...
            <button class="support-link" id="repo" type="button">
              ...
              <span class="summary-detail">Browse the upstream KennyHarness project.</span>
            </button>
            <button class="support-link" id="commands" type="button">
              <span class="support-link-label">Open Command Palette</span>
              <span class="summary-detail">Access VS Code and KennyHarness commands quickly.</span>
            </button>
          </div>
        </section>
      </section>
...
class KennyHarnessControlCenterProvider {
  constructor() {
  ...
  async resolveWebviewView(webviewView) {
    ...
    webviewView.webview.onDidReceiveMessage(async message => {
      switch (message?.type) {
        case 'launch':
          await launchKennyHarness();
          break;
        case 'launchRoot':
          await launchKennyHarness({ requireWorkspace: true });
          break;
        case 'openProfile':
...
function activate(context) {
  ...
  const provider = new KennyHarnessControlCenterProvider();
  ...
  const chatViewProvider = new KennyHarnessChatViewProvider(chatController);
  const chatPanelManager = new KennyHarnessChatPanelManager(chatController);
  ...
  statusBarItem.text = '$(comment-discussion) KennyHarness';
  statusBarItem.tooltip = 'Open KennyHarness Chat';
  statusBarItem.command = 'kennyharness.openChat';
  statusBarItem.show();

  chatController.onDidChangeState((state) => {
    switch (state) {
      case 'streaming':
        statusBarItem.text = '$(sync~spin) KennyHarness';
        statusBarItem.tooltip = 'KennyHarness is generating...';
        break;
      case 'connected':
        statusBarItem.text = '$(comment-discussion) KennyHarness';
        statusBarItem.tooltip = 'KennyHarness connected';
        break;
      default:
        statusBarItem.text = '$(comment-discussion) KennyHarness';
        statusBarItem.tooltip = 'Open KennyHarness Chat';
  ...
  // ── Existing commands ──
  const startCommand = vscode.commands.registerCommand('kennyharness.start', async () => {
    await launchKennyHarness();
  });

  const startInWorkspaceRootCommand = vscode.commands.registerCommand(
    'kennyharness.startInWorkspaceRoot',
    async () => {
      await launchKennyHarness({ requireWorkspace: true });
    },
  );

  const openDocsCommand = vscode.commands.registerCommand('kennyharness.openDocs', async () => {
    await vscode.env.openExternal(vscode.Uri.parse(KENNYHARNESS_REPO_URL));
  });

  const openSetupDocsCommand = vscode.commands.registerCommand(
    'kennyharness.openSetupDocs',
    async () => {
      await vscode.env.openExternal(vscode.Uri.parse(KENNYHARNESS_SETUP_URL));
    },
  );
  ...
  const openWorkspaceProfileCommand = vscode.commands.registerCommand(
    'kennyharness.openWorkspaceProfile',
    async () => {
      await openWorkspaceProfile();
    },
  );

  const openUiCommand = vscode.commands.registerCommand('kennyharness.openControlCenter', async () => {
    await vscode.commands.executeCommand('workbench.view.extension.kennyharness');
  });

  // ── New chat commands ──
  const newChatCommand = vscode.commands.registerCommand('kennyharness.newChat', () => {
    chatController.stopSession();
    chatController.broadcast({ type: 'session_cleared' });
  });

  const openChatCommand = vscode.commands.registerCommand('kennyharness.openChat', () => {
    chatPanelManager.openPanel();
  });

  const resumeSessionCommand = vscode.commands.registerCommand('kennyharness.resumeSession', async () => {
  ...
  const abortChatCommand = vscode.commands.registerCommand('kennyharness.abortChat', () => {
    chatController.abort();
  });

  // ── Register providers ──
  const controlCenterProviderReg = vscode.window.registerWebviewViewProvider(
    'kennyharness.controlCenter',
    provider,
  );
  ...
  context.subscriptions.push(
    ...
    vscode.workspace.onDidChangeConfiguration(event => {
      if (event.affectsConfiguration('kennyharness')) {
        refreshProvider();
      }
    }),
...
module.exports = {
  activate,
  deactivate,
  KennyHarnessControlCenterProvider,
  renderControlCenterHtml,
  resolveLaunchTargets,
  ChatController,
  KennyHarnessChatViewProvider,
  KennyHarnessChatPanelManager,
};
