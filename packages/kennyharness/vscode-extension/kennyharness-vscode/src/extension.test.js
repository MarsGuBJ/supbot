...
function createStatus(overrides = {}) {
  return {
    installed: true,
    executable: 'kennyharness',
    launchCommand: 'kennyharness --project-aware',
    terminalName: 'KennyHarness',
    shimEnabled: false,
    workspaceFolder: '/workspace/kennyharness/very/long/path/example-project',
    workspaceSourceLabel: 'active editor workspace',
    launchCwd: '/workspace/kennyharness/very/long/path/example-project',
    launchCwdLabel: '/workspace/kennyharness/very/long/path/example-project',
    canLaunchInWorkspaceRoot: true,
    profileStatusLabel: 'Found',
    profileStatusHint: '/workspace/kennyharness/very/long/path/example-project/.kennyharness-profile.json',
    workspaceProfilePath: '/workspace/kennyharness/very/long/path/example-project/.kennyharness-profile.json',
    providerState: {
      label: 'Codex',
      detail: 'gpt-5.4',
      source: 'profile',
...
test('renderControlCenterHtml uses the KennyHarness wordmark, status rail, and warm action hierarchy', () => {
  ...
  assert.match(
    html,
    /title="\/workspace\/kennyharness\/very\/long\/path\/example-project"[^>]*>\/workspace\/kennyharness\/very\/long\/path\/example-project<\//,
  );
});
...
test('KennyHarnessControlCenterProvider.getHtml supplies a nonce to the redesigned renderer', () => {
  const { KennyHarnessControlCenterProvider } = loadExtension();
...
test('resolveLaunchTargets distinguishes project-aware launch from workspace-root launch', () => {
  ...
  assert.deepEqual(
    resolveLaunchTargets({
      activeFilePath: '/workspace/kennyharness/src/panels/control-center.js',
      workspacePath: '/workspace/kennyharness',
      workspaceSourceLabel: 'active editor workspace',
    }),
    {
      projectAwareCwd: '/workspace/kennyharness/src/panels',
      projectAwareCwdLabel: '/workspace/kennyharness/src/panels',
      projectAwareSourceLabel: 'active file directory',
      workspaceRootCwd: '/workspace/kennyharness',
      workspaceRootCwdLabel: '/workspace/kennyharness',
...
test('resolveLaunchTargets anchors relative launch commands to the workspace root', () => {
  ...
  assert.deepEqual(
    resolveLaunchTargets({
      executable: './node_modules/.bin/kennyharness',
      activeFilePath: '/workspace/kennyharness/src/panels/control-center.js',
      workspacePath: '/workspace/kennyharness',
      workspaceSourceLabel: 'active editor workspace',
    }),
    {
      projectAwareCwd: '/workspace/kennyharness',
      projectAwareCwdLabel: '/workspace/kennyharness',
...
test('renderControlCenterHtml explains distinct launch targets when an active file directory is available', () => {
  ...
  const html = renderControlCenterHtml(
    createStatus({
      launchCwd: '/workspace/kennyharness/src/panels',
      launchCwdLabel: '/workspace/kennyharness/src/panels',
  ...
  assert.match(html, /Starts beside the active file · \/workspace\/kennyharness\/src\/panels/);
  assert.match(html, /Always starts at the workspace root · \/workspace\/kennyharness/);
});
...
test('renderControlCenterHtml makes shared workspace-root launches explicit for relative commands', () => {
  ...
  const html = renderControlCenterHtml(
    createStatus({
      launchCwd: '/workspace/kennyharness',
      launchCwdLabel: '/workspace/kennyharness',
  ...
  assert.match(html, /Project-aware launch is anchored to the workspace root by the relative command · \/workspace\/kennyharness/);
  assert.match(html, /Same workspace-root target as Launch KennyHarness because the relative command resolves from the workspace root · \/workspace\/kennyharness/);
});
