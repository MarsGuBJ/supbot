 ...
 *   ~/.kennyharness/projects/<sanitized-cwd>/<sessionId>.jsonl
 *
...
function resolveConfigDir() {
  ...
  const kennyHarnessDir = path.join(home, '.kennyharness');
  const legacyDir = path.join(home, '.claude');
  if (!fs.existsSync(kennyHarnessDir) && fs.existsSync(legacyDir)) {
    return legacyDir;
  }
  return kennyHarnessDir;
}
